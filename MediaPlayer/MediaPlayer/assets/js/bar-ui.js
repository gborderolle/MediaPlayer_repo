...
(function (window) {
    ...
    var Player,
        ...
        playerSelector = '.sound_player_class',
        playerOptions,
        utils;
  ...
  Player = function(playerNode) {
    ...
    */
    ...
    function getTime(msec, useString) {
      ...
      function getSeconds(msec, useString) {
        ...
        whileplaying: function() {
          ...
          //if (left != null && !isNaN(left)) {
              ...
              if (this.duration) {
                  ...
                  // http://stackoverflow.com/questions/3942776/using-jquery-to-find-an-element-at-a-particular-position

                  // 1- avance dentro de la duracion
                  // 2- startdate + avance (av_startdate)


                  // IMPORTANT: GET SCREEN RECORDINGS ELEMENTS IN RANGE -- Currently Disabled

                  /*
                  if (selectedElementID != null && selectedElementID > 0) {
                      var element = getElementInMemoryByID(selectedElementID);
                      if (element != null) {
                          var datetime = element.timestamp;
                          if (datetime != null) {

                              var myDateStart1 = moment(datetime, "dd-MM-yyyy hh:mm:ss");
                              var myDateEnd1 = moment(datetime, "dd-MM-yyyy hh:mm:ss");
                              if (myDateStart1 != null && myDateEnd1) {



                                  // Add seconds to startDate
                                  var secs = getSeconds(this.position, true);
                                  myDateEnd1.add(secs, "seconds");

                                  var myDateStart2 = myDateStart1.toDate();
                                  var myDateEnd2 = myDateEnd1.toDate();
                                  ...
                                  // AT THIS MOMENT !!!
                                  var elementsCandidate = getElementInMemoryByTimeRange_onlyVisibleInTimeline(selectedElementID, myDateStart2, myDateEnd2);
                                  if (elementsCandidate != null && elementsCandidate.length > 0) {
                                      //for (var i = 0; i < elementsCandidate.length; i++) {
                                      var el = elementsCandidate[0];
                                      if (el != null) {
                                          ...
                                          //console.log(el.tapeType + " - " + el.fileName);
                                          el.isPlaying = "true";

                                          // AUDIO ELEMENT -------------------------------
                                          if (el.tapeType === "A") {
                                              $("#divPlayer_AUDIO").show("clip", 1800);
                                              $("#lnkSound_AUDIO").attr("href", el.fileName);

                                              if (divControlsMask_AUDIO != null && divControlsMask_AUDIO.length > 0) {
                                                  if (divControlsMask_AUDIO.hasClass("playing")) {
                                                      divControlsMask_AUDIO.addClass("pause");
                                                      divControlsMask_AUDIO.removeClass("playing");

                                                      var pointer = $('#sm2-progress-ball_AUDIO'); // both
                                                      if (pointer != null) {
                                                      ...
                                                      // Set element Playing
                                                      setElementInMemoryIsPlaying(el.tapeID);
                                                  }
                                              }

                                          }
                                              // SCREEN RECORDNG ELEMENT -------------------------------
                                          else if (el.tapeType === "S") {
                                              ...
                                              // Video player
                                              $("#sm2-progress-track_VIDEO").on("click", { _duration: el.duration, _d: $(this) }, setVideoCurrent);

                                              // Bottom progress track
                                              $("#sm2-progress-track").on("click", { _duration: el.duration }, setVideoCurrent);

                                              // Click over play/pause button 
                                              //divControlsMask_AUDIO.on("click", { _tapeID: el.tapeID }, playAudioElement);

                                              // Clear on click events
                                              aPlayPause_VIDEO.attr('onclick', '');
                                              aPlayPause_VIDEO.off("click");

                                              // Click on Play icon VIDEO
                                              $("#aPlayPause_VIDEO").bind("click", function () {
                                                  console.log("click l: 299 (bar-ui.js)");
                                              ...
                                              // Click on Play icon VIDEO
                                              $("#aPlayPause_VIDEO").bind("click", function () {
                                                  console.log("click l: 333 (bar-ui.js)");
                                                  if (divControlsMask_VIDEO != null && divControlsMask_VIDEO.length > 0) {
    ...
    function handleMouseDown(e) {
      ...
      if (utils.css.has(target, 'sm2-volume-control')) {
        ...
        utils.events.add(document, 'mousemove2', actions.adjustVolume);
    ...
    function handleMouse(e, target) { // MouseMove

    //var target, barX, barWidth, x, newPosition, sound;
    var barX, barWidth, x, newPosition, sound;
    ...
    function releaseMouse(e) {
      ...
      utils.css.remove(dom.o2, 'grabbing');
    ...
    function init() {
      ...
      //$("#divControlsMask_VIDEO").off("click"); // Remove event
      ...
      utils.events.add(dom.progressTrack, 'mousedown', function (e) {
          ...
          return handleMouse(e, dom.progressTrack);

      });

        // bottom progress track
        /*
      utils.events.add(dom.progressTrack2, 'mousedown', function (e) { 
          if (isRightClick(e)) {
              return true;
          }

          utils.css.add(dom.o2, 'grabbing'); // +++++++++
          utils.events.add(document, 'mousemove', handleMouse);
          utils.events.add(document, 'mouseup', releaseMouse);

          return handleMouse(e, dom.progressTrack2);
      });
      */
    }
    // ---
  ...
  //window.soundObject1 = soundObject1;
